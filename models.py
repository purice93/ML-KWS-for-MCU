...
def create_attention_model(fingerprint_input, model_settings, model_size_info,
    ...
    with tf.name_scope('cnn'):
        if use_cnn:
            ...
            cnn_input = tf.expand_dims(fingerprint_4d, -1)
            ...
            cnn_dropout = tf.nn.dropout(
                cnn_res, cnn_dropout_keep_prob)
            # Flatten.
            output_height = math.floor((input_time_size - 20 + 1) / 1)
            output_width = math.floor((input_frequency_size - 5 + 1) / 2)
